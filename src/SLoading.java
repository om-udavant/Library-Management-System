...
public class SLoading extends JFrame implements Runnable {
		...
		try {
			for (int i = 0; i < 200; i++) {
				...
				if (v < m) {
					...
					i = 201;
					setVisible(false);
				    new StudentHome(number).setVisible(true);
				}
				Thread.sleep(50);
			}
		} catch (Exception e) {
